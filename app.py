...
def load_user(username):
    ...
    with conn.cursor() as cur:
        ...
        user = cur.fetchone()
        if user:
            return User(username=user[0], rol=user[1], acceso_ventas=user[2], acceso_caja=user[3], acceso_inventario=user[4])
    conn.close()
    return None
...
# def limpiar_usuarios_en_uso():
    while True:
        ...
        cur.execute("""
            ...
            SET EN_USO = 'N'
            WHERE EN_USO = 'S'
            AND (SYSDATE - FECHA_INICIO_SESION) * 1440 > 60
        """)
        conn.commit()
        conn.close()
        time.sleep(60) 

# threading.Thread(target=limpiar_usuarios_en_uso, daemon=True).start()
...
def verificar_estado_usuario():
    if current_user.is_authenticated:
        ...
        cur = conn.cursor()
        cur.execute("""
            SELECT EN_USO FROM ADMIN.USUARIOS WHERE USUARIO = :1
        ...
        if not estado or estado[0] != 'S':
            logout_user()
            session.clear() 
            flash("Tu sesión ha expirado o fue cerrada por otro inicio de sesión.", "warning")
            return redirect(url_for('login'))
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            with conn.cursor() as cur:
                ...
                cur.callproc("INICIO_SESION", [Usuario, Contraseña, out_cursor])
                result = out_cursor.getvalue().fetchone()

                print("DB -> Result:", repr(result))
                
                if result and result[5] == 'S':
                    flash('El usuario ya está en uso, por favor cierra la sesión en otro dispositivo.', 'danger')
                    return render_template('index.html')

                if result:
                    user = User(username=result[0], rol=result[1], acceso_ventas=result[2], acceso_caja=result[3], acceso_inventario=result[4])
                    login_user(user)
...
def vaciar_carrito():
    ...
    conn = get_connection()
    with conn.cursor() as cur:
        # Actualiza cada producto individualmente con su cantidad correcta
        for item in carrito:
            cur.execute("""
...
def carrito_eliminar(codigo):
    ...
    # Obtiene el valor directamente, no una lista
...
def realizar_venta():
    carrito = session.get("carrito", [])
    print("Carrito para venta:", carrito)
...
def agregar_producto():
    ...
    print('Productos Seleccionados:', seleccionados)
    ...
    conn = get_connection()
    with conn.cursor() as cur:
        placeholders = ','.join([':{}'.format(i+1) for i in range(len(seleccionados))])
        
        cur.execute(f"""
            SELECT CODIGO, 
                   CASE 
                       WHEN CANTIDAD = 0 THEN 'sin_stock'
                       WHEN EN_USO = 'S' THEN 'en_uso'
                   END AS motivo
            FROM ADMIN.PRODUCTO 
            WHERE CODIGO IN ({placeholders}) 
            AND (CANTIDAD = 0 OR EN_USO = 'S')
        """, seleccionados)
        ...
        if bloqueados:
            ...
            for row in bloqueados:
                codigo, motivo = row[0], row[1]
                if motivo == 'sin_stock':
                    mensajes.append(f"{codigo} (sin stock)")
                else:
                    mensajes.append(f"{codigo} (en uso)")
            
            flash(f"Productos no disponibles: {', '.join(mensajes)}", "danger")
            session['bloqueados'] = [row[0] for row in bloqueados]
            return redirect(url_for('ventas'))
    ...
    for p in productos:
        codigo = p[4]
        cantidad_seleccionada = int(request.form.get(f'cantidad_{codigo}', 1))
        producto_dict = {
            "nombre": p[0],
            "categoria": p[1],
            "precio": p[2],
        ...
        if not any(item['codigo'] == codigo for item in session['carrito']):
            session['carrito'].append(producto_dict)
            
    with conn.cursor() as cur:
        cur.execute(f"""
            UPDATE ADMIN.PRODUCTO SET CANTIDAD = CANTIDAD - ({cantidad_seleccionada})
            WHERE CODIGO IN ({placeholders})
            AND CANTIDAD > 0 
            AND EN_USO = 'N'
        """, seleccionados)
...
def api_ventas_caja():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
                SELECT * FROM ADMIN.VIEW_VENTAS_CAJA
        ...
        return jsonify([
            {
                ...
                "descripcion": row[3],
                "fecha": str(row[4]),
                "hora": row[5].strftime('%H:%M') if hasattr(row[5], 'strftime') else row[5]
            }
            for row in rows
        ])
        ...
        with conn.cursor() as cur:
            cur.execute("""
                SELECT pf.ID_GF, 
                       ...
                       f.FECHA,
                       (SELECT HORA FROM ADMIN.HORA WHERE ID_HORA = pf.HORA FETCH FIRST 1 ROWS ONLY) AS HORA
                ...
                ORDER BY TO_DATE(f.FECHA, 'DD/MM/RR') DESC, HORA DESC
            """, {
                "fecha_inicio": fecha_inicio_fmt,
                "fecha_fin": fecha_fin_fmt
            })
...
def api_filtro_gastos_caja():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("""
    SELECT * FROM ADMIN.FILTRO_GASTOS 
    WHERE FECHA IS NOT NULL
    AND TO_DATE(FECHA, 'DD/MM/RR')
        BETWEEN TO_DATE(:fecha_inicio, 'DD/MM/RR')
            AND TO_DATE(:fecha_fin, 'DD/MM/RR')
...
def actualizar_inventario():
    codigo = request.form.get('codigo')  # Código del producto
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM ADMIN.PRODUCTO WHERE UPPER(NOMBRE) = UPPER(:1) AND UPPER(T_UNIDAD) = UPPER(:2)", (nombre, unidad))
    contar = cursor.fetchone()[0]
    if contar > 2:
        flash("El producto ya existe, Verifique su disponibilidad en el inventario.", "danger")
        return redirect(url_for('inventario'))
    cursor.execute(""" 
        INSERT INTO ADMIN.MERMA (CANTIDAD)
        VALUES (:1)""", (merma,))
    cursor.execute("SELECT MAX(ID_M) FROM ADMIN.MERMA")
    Mermas = cursor.fetchone()
    cursor.execute("""
        UPDATE ADMIN.PRODUCTO
        SET NOMBRE = :1, PRECIO = :2, CANTIDAD = :3, T_UNIDAD = :4, MERMA = :5, CATEGORIA = :6
        WHERE CODIGO = :7
    """, (nombre, precio, cantidad, unidad, Mermas[0], categoria, codigo))
...
""", (nombre, contrasena, rol, estado, filename, acceso_ventas, acceso_caja, acceso_inventario))
...
def editar_usuario():
    ...
    WHERE ID_USUARIO = :9
""", (nombre, contrasena, rol, estado, filename, acceso_ventas, acceso_caja, acceso_inventario, id_usuario))
